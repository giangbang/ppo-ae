...
if __name__ == "__main__":
    ...
    for i in range(4):
        new_i = goal[0] + di[i]
        new_j = goal[1] + dj[i]
        if new_i >= env.grid.height or new_j >= env.grid.width or \
                new_i < 0 or new_j < 0 or env.grid.get(new_i, new_j) is not None: continue
        agent_goal = (new_i, new_j)
        break
    ...
    mask = np.zeros(record_state.shape, dtype=float)
