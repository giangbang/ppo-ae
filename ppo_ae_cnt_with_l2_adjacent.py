        ...
        for step in range(0, args.num_steps):
            ...
            if global_step > args.ae_warmup_steps:
                with torch.no_grad():
                    ...
                    mask = 1 - done
                    latent_distance = ((mask*(prev_embedding - next_embedding)) ** 2).sum(dim=-1)

                count_reward = torch.sqrt(1/(count_in_ball+1))

                latent_reward = intrinsic_rw(latent_distance, args.l2_clip)*count_reward
                latent_reward = args.adv_rw_coef * latent_reward.view(rewards[step].shape)

                intrinsic_reward = args.adv_rw_coef * latent_reward.view(rewards[step].shape)
